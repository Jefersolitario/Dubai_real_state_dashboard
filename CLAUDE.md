...
6. **Streamlit UI** - three pages via `st.segmented_control` (not `st.tabs`, which executes every tab body on each rerun): "Market Overview" (`_render_market_overview`: headline KPIs, Dubai Market Pulse, tier trends, opportunity scanner), "Zone Analysis" (`_render_zone_analysis`: neighbourhood-filtered price charts and the raw-data table — only rendered when selected), and "Fair Value Model" (`render_fair_value_tab` from `fair_value_tab.py`, lazy — model bundle load and scoring only run when this page is selected); shared sidebar filters.
