...
6. **Streamlit UI** - three pages via `st.segmented_control` (not `st.tabs`, which executes every tab body on each rerun): "Market Overview" (`_render_market_overview`: headline KPIs, Dubai Market Pulse, tier trends), "Zone Analysis" (`_render_zone_analysis`: single-zone focus with a zone picker and two candidate views being A/B-compared — `_render_zone_deal_finder` (threshold slider, diverging below-median scanner, ranked deals table + CSV, per-zone deal counts) and `_render_zone_classic` (the original four charts scoped to the zone) — plus the raw-data table; only rendered when selected), and "Fair Value Model" (`render_fair_value_tab` from `fair_value_tab.py`, lazy — model bundle load and scoring only run when this page is selected); shared sidebar filters.
