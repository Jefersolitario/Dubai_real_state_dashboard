...
- **METER_SALE_PRICE is mechanically derived** from TRANS_VALUE/ACTUAL_AREA
  ...
  are the **project median AED/sqm** (fallbacks: district×rooms, district,
...
- **PROCEDURE_AREA equals ACTUAL_AREA on every row**, so partial-ownership
  ...
  0.25/0.5/0.75 of area, per-share price at comp) is implemented and dormant;
...
| Rule | Action | n | Logic |
|---|---|---|---|
| `price_digit_shift` | repaired | **157** (0.05%) | price/comp ratio in a 10^k zone (k=±1,±2), recorded area credible for the layout (±35% of project×rooms median or a registry layout match), repaired price within ±35% of comp |
...
| `bulk_allocation` | review_only | **1,650** (0.54%) | ≥3 same-project same-day identical prices (or identical AED/sqm to 0.1%) with the group ≥25% below comp. Crucially, **13,294 off-plan developer-launch rows in identical-price batches stay clean** (their median ratio to comp is 0.999 — those are real primary-market prices; excluding them would delete 4% of the market) |
| `suspected_token_transfer` | review_only | **740** (0.24%) | internally consistent price below 40% of comp (−60%+); IAAO calls these "sales of convenience… retransacted at only a nominal price" |
| `extreme_price_unexplained` | review_only | 0 | unrepaired >6.5× comp |
...
>35% from comp are **not** fabricated into existence — they fall through to
...
- The Fair Value tab shows review-routed rows in the **"Excluded suspicious
  records"** expander with plain-language reasons and each row's price-vs-comp
  ratio: the product answer to "how do we deal with these cases" is *show
...
(price typo ×10, area typo ×10, half-share transfer, below-comp bulk trio,
at-market launch trio, token price at 25% of comp) and asserts each fires
