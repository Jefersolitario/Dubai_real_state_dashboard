...
def _render_buyer_guide() -> None:
    """Plain-language buyer guidance, written as the questions people ask."""
    with st.expander("🎯 New here? How to use this page", expanded=False):
        st.markdown(
            "**What this page does.** Every month about 10,000 apartments change "
            "hands in Dubai. This page estimates what each one *should* have sold "
            "for, then shows you the ones that sold for noticeably less. It "
            "narrows the whole market down to a handful worth your time — but "
            "the final judgement is yours, in person.\n\n"
            "---\n\n"
            "**“Is a 5% discount a good deal?”**\n\n"
            "No. The estimate itself is typically about 4% out, so anything under "
            "roughly 8% below fair value is normal wobble rather than a bargain. "
            "Small discounts are noise.\n\n"
            "**“Which one should I look at first?”**\n\n"
            "The top of the list. It is sorted by how *unusual* the price is for "
            "that kind of sale, not simply by the biggest discount. A 15% discount "
            "in a busy building where hundreds of similar flats have sold is strong "
            "evidence; the same 15% in a building that rarely trades means much "
            "less, because there is little to compare against.\n\n"
            "**“What does ‘distressed’ mean?”**\n\n"
            "That there is a second reason for the low price, beyond the estimate's "
            "opinion. Usually it means several owners were selling at once (often an "
            "inheritance, a divorce, or partners splitting up) or the building almost "
            "never trades. A low price on its own never earns the label — otherwise "
            "one bad estimate would masquerade as a bargain.\n\n"
            "**“Can I buy without viewing it?”**\n\n"
            "No. The estimate knows the size, the building, the floor plan and what "
            ...
            "That means discounts look slightly bigger than they truly are. Lean "
            "towards the flats with a second reason to be cheap rather than simply "
            "the largest discount, and check the market trend on the Market "
            "Overview page before you make an offer.\n\n"
            "---\n\n"
            "**In short: this is a filter, not a verdict.**\n\n"
            "It sifts roughly 10,000 sales a month down to the few hundred that "
            "look underpriced, then down to the handful that have a second reason "
            "to be cheap. That is the part no person could do by hand, and it is "
            "what saves you from viewing the wrong flats. What it cannot do is "
            "stand in the room.\n\n"
            "**The model narrows the market down to a shortlist. "
            "Twenty minutes in the flat does the last 4%.**"
        )
